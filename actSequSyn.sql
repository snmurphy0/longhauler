...
--0) added the U07.1 ICD code to the covidpos table
...
--6) added option #4 to output NO DATES (now in theory a de-identified set)
--7) added option #5 to output NO DATES with a fraction of the patients
...
insert into fource_config
	select 'mgb_act', -- siteid
		...
		'ICD10:', -- code_prefix_icd10cm -- CHANGED FOR ACT
...
create table fource_config2 (
	...
	output_as_csv_fraction_nodates bit -- tables with a single column containing comma separated values with NO DATES
)
insert into fource_config2
	select 
		...
		0, -- output_as_columns
		0, -- output_as_csv WITH DATES
		1, -- output_as_csv with NO DATES
		0  -- output_as_csv fraction of patients (set in option 5) with NO DATES
...
begin
	...
	-- LocalPatientClinicalCourse
	select s actPatientClinicalCourse
		from (
			select 0 i, 'siteid,patient_num,days_since_admission,calendar_date,in_hospital,severe,deceased,encounter_seq' s
			union all 
			select row_number() over (order by patient_num, days_since_admission) i,
	...
	-- LocalPatientObservations
	select s actPatientObservations
		from (
			select 0 i, 'siteid,patient_num,days_since_admission,concept_type,concept_code,value' s
			union all 
			select row_number() over (order by patient_num, concept_type, concept_code, days_since_admission) i,
...
--------------------------------------------------------------------------------
-- OPTION #5: View the data as csv strings, limit to a managable subset of the syn database
-- * Copy and paste to a text file, save it FileName.csv.
-- * Make sure it is not saved as FileName.csv.txt.
--   DON'T INCLUDE ANY DATES
--------------------------------------------------------------------------------
if exists (select * from fource_config2 where output_as_csv_fraction_nodates = 1)
begin
	DECLARE @Patient_num_top_limit INT;
	DECLARE @Patient_num_max INT;
	DECLARE @Patient_num_min INT;
	DECLARE @Patient_num_divide_by INT;

	SET @Patient_num_divide_by = 2;

	select @Patient_num_min=min(patient_num), @Patient_num_max=max(patient_num)
	from OBSERVATION_FACT;

	SET @Patient_num_top_limit = (@Patient_num_max - @Patient_num_min)/@Patient_num_divide_by;

	-- Select @Patient_num_top_limit
	-- Select count(distinct(patient_num)) from OBSERVATION_FACT where patient_num <= @Patient_num_top_limit;

	-- LocalPatientSummary
	select s synPatientSummary
		from (
			select 0 i, 'siteid,patient_num,admission_date,days_since_admission,last_discharge_date,still_in_hospital,'
				+'severe_date,severe,death_date,deceased,sex,age_group,race,race_collected' s
			union all 
			select row_number() over (order by admission_date, patient_num) i,
				...
				+','+cast(race_collected as varchar(50))
			from fource_LocalPatientSummary
			where patient_num <= @Patient_num_top_limit
			union all select 9999999, '' --Add a blank row to make sure the last line in the file with data ends with a line feed.
		) t
		order by i

	-- LocalPatientClinicalCourse
	select s synPatientClinicalCourse
		from (
			select 0 i, 'siteid,patient_num,days_since_admission,calendar_date,in_hospital,severe,deceased,encounter_seq' s
			union all 
			select row_number() over (order by patient_num, days_since_admission) i,
	...
	-- LocalPatientObservations
	select s synPatientObservations
		from (
			select 0 i, 'siteid,patient_num,days_since_admission,concept_type,concept_code,value' s
			union all 
			select row_number() over (order by patient_num, concept_type, concept_code, days_since_admission) i,
	...
	-- LocalPatientMapping
	select s synLocalPatientMapping
		from (
			select 0 i, 'siteid,patient_num,study_num' s
			union all 
			select row_number() over (order by patient_num) i,
